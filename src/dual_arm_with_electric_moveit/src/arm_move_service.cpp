...
public:
  ArmMoveService() : Node("arm_move_service")
  {
    ...
    RCLCPP_INFO(this->get_logger(), "   - r_arm: 仅控制右臂");
    RCLCPP_INFO(this->get_logger(), "   - l_arm: 仅控制左臂");
    RCLCPP_INFO(this->get_logger(), "   - both_arms: 同时控制双臂");
  }
...
private:
    ...
    std::shared_ptr<dual_arm_with_electric_moveit::srv::MoveArm::Response> response)
  {
    ...
    if (request->control_mode == "r_arm")
    {
      ...
      auto result = move_single_arm("r_arm", request->target_pose_right);
    ...
    else if (request->control_mode == "l_arm")
    {
      ...
      auto result = move_single_arm("l_arm", request->target_pose_left);  // 修改：这里改成 l_arm
    ...
    else
    {
      ...
      RCLCPP_ERROR(this->get_logger(), 
                   "❌ 无效的控制模式: %s (应为 'r_arm', 'l_arm' 或 'both_arms')", 
                   request->control_mode.c_str());
      response->success = false;
